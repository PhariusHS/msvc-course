...
@Service
@Transactional
public class CreateUserService implements RelationalCommand<UserDTO, Long, UserDTO> {

    private final CourseRepository courseRepository;
    private final UserClientRest userClientRest;


    public CreateUserService(CourseRepository courseRepository, UserClientRest userClientRest){
    ...
    public ResponseEntity<UserDTO> execute(UserDTO userDTO, Long courseId) {
        ...
        if(foundedCourse.isPresent()){
            UserDTO msvcUser = userClientRest.createUser(userDTO);
